...
- **多连接管理与远程连接**：设置页统一管理多个 Docker 连接（Linux 支持本地 socket / SSH / TLS / 明文 TCP；Windows 仅支持 SSH / TLS / 明文 TCP），支持连通性测试（延迟与版本）、添加 / 编辑 / 删除、一键切换并自动刷新数据；侧栏底部可快速切换当前连接。SSH 连接由应用自动建立加密隧道（支持指定私钥与远程 rootless socket 路径），TLS 走客户端证书双向认证；配置方法与故障排查见「远程连接」章节
